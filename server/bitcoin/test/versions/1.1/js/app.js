...
*
*/
version("Pre-release: v.1.1.14");
/*
*
* Pre-release: v.1.1
* + добалено уведомления на телеграм
* + добалено стратегия на LINK
* + добалено стратегия по объему торгов Volume (test)
* + добалено стратегия на покупку при падение цены (scalp_BUY_SHORT)
* + добалено strategybar
*
*
* Pre-alpha: v.1.2.0
*   TODO:
* - admin-панель
* - функция по рекомендаций на покупку (BUY -> $3 ETH)
...
* - BTC/USDT scalp_BUY_SHORT: -037% до -0.50%
* - SOL/USDT scalp_BUY_SHORT: -0.60% до -0.90%
* - ETH/USDT scalp_BUY_SHORT: -0.40%(-0.54%) до -0.80%
* - LINK/USDT scalp_BUY_SHORT: -0.50% до -0.80%
* - FTT/USDT scalp_BUY_LONG: 0.77 %, scalp_BUY_SHORT: -0.70%  до -1.20%, INDICATOR_vol: 22к
* - DOT/USDT scalp_BUY_LONG: 0.60%,  scalp_BUY_SHORT: -0.50%(или 0.65%) до -0.??%
* - гугл таблыца
* -  
*/
...
*  6.1. если последные 15 мин есть свечи (или последние 3шт.) с болышими минусами (-0.70%)(или обемов), не покупать.
...
*  12. если объем  торгов по SOL (30к), BTC (240к)но не более 20m, LINK (25к) - но нужно розсчитать аплитуду(2x)
*  13. если 
*
*
*
...
* Используйте очеловеченные, объясняющие  названия
* Функция должна решать одну задачу
...
const signal = (symbol, data, vol) => {
    ...
    try {
        ...
        console.log(s, strategyBUY().min_SHORT, data, strategyBUY().max_SHORT);
        
        if (getRange(strategyBUY().min_LONG, data, strategyBUY().max_LONG)) { // стратегия по scalp_BUY_LONG
            console.log(s + " SIGNAL: " + data + ", Range: " + strategyBUY().max_LONG + ", 60 min = " + lastBUY(BUY, timeDate()) + " min");
        ...
        if (getRange(strategyBUY().min_SHORT, data, strategyBUY().max_SHORT)) { // стратегия по scalp_BUY_SHORT
            soundNotification();
            getTelegram(s + " (SHORT");
            return "BUY SHORT";
        } else {
